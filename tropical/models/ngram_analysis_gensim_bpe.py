...
from tokenizers import (ByteLevelBPETokenizer,  # noqa
                        CharBPETokenizer,  # noqa
                        SentencePieceBPETokenizer,  # noqa
                        BertWordPieceTokenizer)  # noqa


class NGramAnalysisGensimBPE(NGramAnalysisBase):
    """
    ...
    def __init__(self) -> None:
        ...
        self.__basic_stopwords: Set[str] = set()  # Start with language-agnostic / empty set.
    ...
    def __tokenize_and_lower(utterances,
                             delimiter: bytes = b'_'):
        """ Expects to train the tokenizer from the entire list of utterances to be analysed. """
        with open("utterances.txt", 'w') as f:
            for utterance in utterances:
                f.write(f"{utterance.lower()}\n")
        # ToDo: Split the tokenizer's training and encoding steps. Train on data over all time frames.
        # ToDo: Experiment with different vocab sizes.
        spm.SentencePieceTrainer.Train('--input=utterances.txt --model_type=bpe --model_prefix=m --vocab_size=1500 '
                                       '--normalization_rule_name=nmt_nfkc')
        sp = spm.SentencePieceProcessor()
        sp.Load("m.model")
        return [sp.EncodeAsPieces(utterance.lower()) for utterance in utterances]

        # https://github.com/huggingface/tokenizers
        # tokenizer = SentencePieceBPETokenizer()  # (replacement=delimiter.decode(encoding="utf-8"))
        # logging.info(f"ngram_analysis_gensim_bpe.__tokenize_and_lower: tokenizer.train...")
        # tokenizer.train(["utterances.txt"], vocab_size=1500, show_progress=False)
        # logging.info(f"ngram_analysis_gensim_bpe.__tokenize_and_lower: tokenizer.encode & respond...")
        # return [tokenizer.encode(utterance.lower()).tokens for utterance in utterances]
