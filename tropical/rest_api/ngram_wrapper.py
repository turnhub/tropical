...
def __ngram_task_and_callback(df: pd.DataFrame, file_format_version: str,
    ...
    try:
        ...
        logging.error(f"ngram_wrapper.__ngram_task_and_callback: requests.Timeout {e}!")
        response_status_code, response_text = 400, json.dumps({"error": f"Callback request timeout ({e})."})
    except requests.RequestException as e:
        logging.error(f"ngram_wrapper.__ngram_task_and_callback: requests.RequestException {e}!")
        response_status_code, response_text = 500, json.dumps({"error": f"Callback request exception ({e})."})

    logging.info(f"ngram_wrapper.__ngram_task_and_callback: Completed {task_uuid} with callback={callback}.")
