"""
Service wrapper for the semantic_cluster model - Gensim implementation.
"""
...
def __semantic_cluster_task(df: pd.DataFrame, file_format_version: str):
    analyser = semantic_cluster_analysis.SemanticClusterAnalysis()
    response_frames = analyser.analyse_dataframe(df)
    return response_frames


def __semantic_cluster_task_and_callback(df: pd.DataFrame, file_format_version: str,
                                         task_uuid: str, callback: str):
    print(f"Process FORKED!!! Now in process with ID={os.getpid()}")

    response_frames = __semantic_cluster_task(df, file_format_version)
    payload = {
        "uuid": task_uuid,
    ...
    try:
        ...
        logging.error(f"semantic_cluster_wrapper.__semantic_cluster_task_and_callback: requests.Timeout {e}!")
        response_status_code, response_text = 400, json.dumps({"error": f"Callback request timeout ({e})."})
    except requests.RequestException as e:
        logging.error(f"semantic_cluster_wrapper.__semantic_cluster_task_and_callback: requests.RequestException {e}!")
        response_status_code, response_text = 500, json.dumps({"error": f"Callback request exception ({e})."})

    logging.info(f"semantic_cluster_wrapper.__semantic_cluster_task_and_callback: Completed {task_uuid} with callback={callback}.")
...
def __start_process(task_filename: str,
    ...
    logging.info(f"semantic_cluster_wrapper.start_process: Start {task_uuid} with callback={callback}.")
    ...
    if callback is None:
        ...
        response_frames = __semantic_cluster_task(df, file_format_version)
        logging.info(f"semantic_cluster_wrapper.__start_process: Completed {task_uuid} with callback={callback}.")
        response_status = 200
    else:
        # Start an async task ...
        print(f"Process WILL BE FORKED! Now in process with ID={os.getpid()}")
        p = Process(target=__semantic_cluster_task_and_callback, args=(df, file_format_version, task_uuid, callback))
        p.start()
